...
public class DealerBranchDao extends SimpleJdbcDaoSupport
{
	...
	int rowsAffected = getJdbcTemplate().update(sql, branch.getGroupCode(), branch.getCode(), branch.getName());
	...
	try
	{
	    ...
	    return getJdbcTemplate().queryForObject(sql, new BeanPropertyRowMapper<>(DealerBranchEntity.class), code);
	} 
